...
import { LABEL_COLORS, ALL_LABELS, CASE_TYPES, COURT_NUMBERS, SITTING_TIMES, fmtDate, fmtDT, getDaysUntil, hearingLabel } from "../caseHelpers";
...
export function BellPanel({ cases, onClose, onSelectCase }: { cases: any[]; onClose: () => void; onSelectCase: (c: any) => void }) {
    ...
    const upcoming = cases.filter(c => { const d = getDaysUntil(c.nextHearingDate); return d !== null && d >= 0 && c.status !== "Disposed" && !c.archived; }).sort((a, b) => new Date(a.nextHearingDate).getTime() - new Date(b.nextHearingDate).getTime());
    return (
        <div ref={ref} style={{ position: "absolute", top: "calc(100% + 8px)", right: 0, width: 340, background: T.surface, border: `1px solid ${T.border}`, borderRadius: 14, boxShadow: "0 12px 40px rgba(15,28,63,0.18)", zIndex: 300, overflow: "hidden" }} role="dialog" aria-label="Upcoming hearings">
            <div style={{ padding: "14px 16px", borderBottom: `1px solid ${T.borderSoft}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div>
                    <div style={{ fontWeight: 800, fontSize: 16, color: T.text }}>Upcoming Hearings</div>
                    <div style={{ fontSize: 13, color: T.textMuted, marginTop: 1 }}>{upcoming.length} scheduled</div>
                </div>
            ...
            <div style={{ maxHeight: 380, overflowY: "auto", padding: "10px 12px" }}>
                {upcoming.length === 0
                    ...
                    : upcoming.map(c => {
                        ...
                        return (
                            <div key={c.id} onClick={() => { onSelectCase(c); onClose(); }} tabIndex={0} role="button"
                                style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px", borderRadius: 10, marginBottom: 6, cursor: "pointer", border: `1px solid ${days != null && days <= 3 ? "#E8D18A" : T.borderSoft}`, background: days != null && days <= 3 ? "#FFFBF0" : T.bg, transition: "background 0.15s" }}
                                onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = days != null && days <= 3 ? "#FFF6DC" : "#E8EBF5"}
                                onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = days != null && days <= 3 ? "#FFFBF0" : T.bg}
                                onKeyDown={e => e.key === "Enter" && (onSelectCase(c), onClose())}>
                                ...
                                <div style={{ flex: 1, minWidth: 0 }}>
                                    <div style={{ fontSize: 14, fontWeight: 700, color: T.accentDark, fontFamily: "Georgia,serif", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{c.caseNumber}</div>
                                    <div style={{ fontSize: 13, color: T.textSub, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{c.petitioners[0]} v. {c.respondents[0]}</div>
                                    <div style={{ fontSize: 13, color: T.textMuted, marginTop: 2 }}>{fmtDate(c.nextHearingDate)} · {c.courtNumber}</div>
                                </div>
                                <span style={{ fontSize: 13, fontWeight: 700, color: col, flexShrink: 0 }}>{hearingLabel(days)}</span>
                            </div>
                        );
                    })}
            </div>
