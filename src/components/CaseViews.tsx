...
function safeDate(raw: string | null | undefined): string {
    ...
    if (/^\d{4}-\d{2}-\d{2}/.test(raw)) return fmtDate(raw);          // ISO format
    const m = raw.match(/(\d{2})-(\d{2})-(\d{4})/);
    if (m) return fmtDate(`${m[3]}-${m[2]}-${m[1]}`);                 // DD-MM-YYYY
    return "No Date";
}
                ...
                return (
                    <div key={c.id} onClick={() => onSelectCase && onSelectCase(c)}
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            <div style={{ fontSize: 13, color: T.textSub, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{c.petitioners[0]}</div>
                        </div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ color: T.text, fontSize: 15.5, marginLeft: 15, marginBottom: 7, lineHeight: 1.45 }}>
                        <span style={{ fontWeight: 600 }}>{hi(c.petitioners[0])}</span>
                        {c.petitioners.length > 1 && <span style={{ color: T.textMuted }}> & {c.petitioners.length - 1} more</span>}
                        <span style={{ color: T.textMuted, fontStyle: "italic" }}> v. </span>
                        <span style={{ color: T.textSub }}>{hi(c.respondents[0])}</span>
                        {c.respondents.length > 1 && <span style={{ color: T.textMuted }}> & {c.respondents.length - 1} more</span>}
                    </div>
                    ...
                    {c.labels?.length > 0 && (
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginLeft: 15, marginTop: 7 }}>
    ...
    return (
        <div onClick={onClick} onKeyDown={e => e.key === "Enter" && onClick()} tabIndex={0} role="button" aria-selected={selected}
            ...
            <div style={{ fontSize: 13, fontWeight: 700, color: T.text, lineHeight: 1.4, flex: 1 }}>
                {c.petitioners[0]}{c.petitioners.length > 1 ? ` +${c.petitioners.length - 1}` : ""}
                <span style={{ color: T.textMuted, fontStyle: "italic", fontWeight: 400 }}> v. </span>
                <span style={{ color: T.textSub, fontWeight: 400 }}>{c.respondents[0]}{c.respondents.length > 1 ? ` +${c.respondents.length - 1}` : ""}</span>
            </div>
                    ...
                    {cases.map((c, i) => {
                        ...
                        return (
                            <tr key={c.id} onClick={() => onSelect(c)}
                                ...
                                <td style={{ padding: "10px 14px", color: T.text, fontWeight: 600, maxWidth: 160, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{hi(c.petitioners[0])}{c.petitioners.length > 1 && <span style={{ color: T.textMuted }}> +{c.petitioners.length - 1}</span>}</td>
                                <td style={{ padding: "10px 14px", color: T.textSub, maxWidth: 140, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{hi(c.respondents[0])}</td>
                                ...
                                return (
                                    <div key={c.id} onClick={() => onSelect(c)}
                                        ...
                                        <div style={{ fontSize: 12, fontWeight: 600, color: T.text, marginBottom: 6, lineHeight: 1.4 }}>
                                            {c.petitioners[0]}<span style={{ color: T.textMuted, fontStyle: "italic", fontWeight: 400 }}> v. </span><span style={{ color: T.textSub, fontWeight: 400 }}>{c.respondents[0]}</span>
                                        </div>
