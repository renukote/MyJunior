...
import { CaseSummarySection, ListingsSection, TimelineSection, TasksSection, NotesSection, DocumentsSection, ApplicationsSection, LexTigressOfficeReportSection, LastOrdersSection, EarlierCourtSection, SCDetailSection } from "./components/DetailSections";
...
(function migrateCases() {
    try {
        ...
        const migrated = cases.map((c: any) => {
            ...
            if (!c.courtName) { c.courtName = 'Supreme Court of India'; changed = true; }
...
const MAIN_NAV = [
    ...
    { id: "causelist", label: "Cause List", icon: "📑" },
...
export default function CourtSync() {
    ...
    useEffect(() => {
        const stored = localStorage.getItem('lextgress_user');
        if (stored) {
    ...
    useEffect(() => {
        discoverMCPTools().then(tools => {
            console.log('[App] MCP tools available:', tools)
        })
    }, []);

    const handleLogout = () => {
        localStorage.removeItem('lextgress_user');
    ...
    useEffect(() => {
        const ok = saveCasesArray(cases);
        if (!ok) setStorageWarn(true);
        const health = checkStorageHealth();
        setStorageHealth(health);
        if (health.isWarning) setStorageWarn(true);
    }, [cases]);
    ...
    function handleCaseFound(caseData: any) {
        ...
        setSuccessToast(`✅ Case ${caseData.caseNumber} added. Upload the Office Report PDF in Team Inbox to generate tasks.`);
    }
        ...
        <AppContext.Provider value={{ T, getS, hearingColor }}>
            <div style={{ display: "flex", height: "100vh", background: T.bg, fontFamily: "'DM Sans','Segoe UI',sans-serif", color: T.text, overflow: "hidden" }}>
                        ...
                        <div style={{ flex: 1, overflow: "auto", padding: "14px 14px 80px" }}>
                            ...
                            <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 12, marginBottom: 14 }}>
                                <div style={{ background: T.bg, borderRadius: 12, border: `1px solid ${T.border}`, padding: 14 }}>
                                    ...
                                    <DR icon="👤" label="Petitioner(s)">{formatParty(selected.petitioner)}</DR>
                ...
                <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
                    ...
                    {activeView !== "cases" && activeView !== "causelist" && activeView !== "tasks" && activeView !== "settings" && activeView !== "ai" && activeView !== "service" && (
                    ...
                    {activeView === "cases" && (
                        <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
                            <div className="main-content-padding" style={{ flex: 1, overflow: "auto", padding: 20 }}>
                                ...
                                {/* SC Lookup */}
                                {!showArchived && <SearchCaseForm onCaseFound={handleCaseFound} theme={T} onViewDetail={handleCasePreview} />}
                                        ...
                                        <div id="case-detail-content" style={{ padding: 20 }}>
                                            ...
                                            <div style={{ marginBottom: 18, paddingBottom: 16, borderBottom: `1px solid ${T.borderSoft}` }}>
                                                ...
                                                <div style={{ display: "flex", gap: 8, marginTop: 14, flexWrap: "wrap", borderTop: `1px solid ${T.borderSoft}`, paddingTop: 14 }}>
                                                    ...
                                                    <button onClick={() => setConfirm({ type: "delete", caseId: selected.id })} style={{ padding: "8px 14px", borderRadius: 9, border: "1px solid #FECACA", background: "#FEF2F2", color: "#C62828", fontSize: 13, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>🗑 Delete</button>
                                                </div>
                                            </div>
                        ...
                        <div style={{ background: T.surface, borderRadius: 16, width: "100%", maxWidth: 800, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 8px 40px rgba(0,0,0,0.35)", position: "relative" }}>
                            ...
                            <div style={{ padding: 24 }}>
                                <div style={{ marginBottom: 20 }}>
                                    ...
                                    <div style={{ fontSize: 16, color: T.textSub, fontStyle: "italic" }}>v. {formatParty(previewCase.respondent)}</div>
                                </div>
